...
#
# Get the admin username and password from keyvault using native Terraform data sources
#

locals {
  admin_username = can(var.settings.virtual_machine_settings["windows"].admin_username_key) ? data.azurerm_key_vault_secret.windows_admin_username.0.value : null
  admin_password = can(var.settings.virtual_machine_settings["windows"].admin_password_key) ? data.azurerm_key_vault_secret.windows_admin_password.0.value : null
}
...
data "azurerm_key_vault_secret" "windows_admin_username" {
  count        = try(var.settings.virtual_machine_settings["windows"].admin_username_key, var.settings.virtual_machine_settings["legacy"].admin_username_key, null) == null ? 0 : 1
  name         = try(var.settings.virtual_machine_settings["windows"].admin_username_key, var.settings.virtual_machine_settings["legacy"].admin_username_key)
  key_vault_id = local.keyvault.id
}
...
data "azurerm_key_vault_secret" "windows_admin_password" {
  ...
  name         = try(var.settings.virtual_machine_settings["windows"].admin_password_key, var.settings.virtual_machine_settings["legacy"].admin_password_key)
  key_vault_id = local.keyvault.id
}
